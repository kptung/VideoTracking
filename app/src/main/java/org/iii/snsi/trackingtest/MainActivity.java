...
public class MainActivity extends Activity {
	...
	/// mCamera is to control camera; mView is to draw the rect when camera is
	/// opened
	...
	private int format = NativeTracking.FORMAT_NV21;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// according to device screen size 2 get the window width and height
		// to display buttons
				...
				if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
					...
					mCamera.setCallbackFrameListener(
							new OldCamera.CallbackFrameListener() {
								@Override
								public void onEncodingTexturePrepared() {
					...
					mCamera2.setFrameFormat(
							CameraController.CallbackFrameListener
									.FRAME_FORMAT_Y);
					mCamera2.setSurfaceHolder(holder);
					mCamera2.openCamera();
					mCamera2.startPreview(true, false, false);
				}
		...
		rec.set((int) ((double) mScreenWidth), (int) ((double) mScreenHeight),
				(int) ((double) mScreenWidth), (int) ((double) mScreenHeight));
		...
		drawThread = new Thread(new Runnable() {
			...
			public void run() {
				while (threadFlag) {
					if (camsaveflag) {
						// rect down-sampling : ( OpenCV Rect (left-top x,
						// left-top y, width, height))
						int lx = Math.round(mView.getmLeftTopPosX() * wRatio);
						int ly = Math.round(mView.getmLeftTopPosY() * hRatio);
						int width = Math.round((mView.getmRightTopPosX() -
								mView
								.getmLeftTopPosX()) * wRatio);
						int height = Math.round(
								(mView.getmRightBottomPosY() - mView
										.getmRightTopPosY()) * hRatio);
						...
						if (objcount == 1) {
							...
							// init tracking
							int[] initarr = tracker.addTrackingObjects(preview,
									previewWidth, previewHeight, roi);
						} else if (objcount > 1) {
							other.add(lx);
							other.add(ly);
							other.add(width);
					...
					if (camtrackflag) {
						...
						if (objcount > 1 && trackcount == 1) {
							...
							int[] addarr = tracker.addTrackingObjects(preview,
									previewWidth, previewHeight, others);
						}
						/// tracking
	...
	private void formatRecognition(int frameFormat) {
		switch (frameFormat) {
			//FRAME_FORMAT_NV21
			case 0:
				format = NativeTracking.FORMAT_NV21;
				break;
			//FRAME_FORMAT_Y
			case 1:
				format = NativeTracking.FORMAT_GRAY;
				break;
			//FRAME_FORMAT_YUV420P
			case 2:
				format = NativeTracking.FORMAT_YUV420P;
				break;
		}
	}
